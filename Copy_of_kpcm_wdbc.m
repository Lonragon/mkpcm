...
%% 开始迭代
 ...
 for i=1:data_n
    for k=1:cluster_n
    for j=1:M
        K(i,k)=exp(((-1)*(data(i,j)-center(k,j))^2)/2*sigma^2);
    end
    end
 end
%  %% Kcom=K1+alpha*K2  (K1,K2都是高斯核函数)%%%%%
...
%               s2(k,j)=s2(k,j)+Um(i,k)*K3(i,k);   
              s1(k,j)=s1(k,j)+Um(i,k)*K(i,k)*data(i,j);  
              s2(k,j)=s2(k,j)+Um(i,k)*K(i,k); 
...
%    index8=find(U(8,:)==max(U));
   ...
   hold on;
%    plot(data(index3,1),data(index3,5),'bo');
%    hold on;
%    plot(data(index4,1),data(index4,5),'m.');
%    hold on;
...
%    hold on;
   title('KPCM wdbc-dataset');
   xlabel('Attribute 1');
   ylabel('Attribute 2');
   plot(center(1,1),center(1,2),'xk','MarkerSize',15);
   plot(center(2,1),center(2,2),'xk','MarkerSize',15);
%    plot(center(3,1),center(3,5),'xk','MarkerSize',15);
%    plot(center(4,1),center(4,5),'xk','MarkerSize',15);
